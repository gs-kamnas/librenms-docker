...
ARG LIBRENMS_VERSION="24.12.0-p1"
...
# Install supercronic as crond requires running as root
# therefore we will use supercronic instead if our EUID at runtime is not 0/root
...
RUN apk --update --no-cache add -t build-dependencies \
  ...
  && mkdir -p /data && chown librenms:librenms /data \
  && apk del build-dependencies \
  && rm -rf .git \
    html/plugins/Test \
